...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  std::smatch match;                
  std::string newSQL;                                                                                  
  std::regex numInjection(" or [1-9]+=[1-9]+;");              //Matches if there are the 1=1 type inputs
  std::regex wordInjection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   //Matches if there are a=a type inputs and/or strings

  //Checks for injections with numbers
  if (regex_search (tempString, match, numInjection)) { 
      std::cout << "SQL Injection Detected: Number match, code removed." << std::endl;
      newSQL = regex_replace(tempString, numInjection, ";");

      return run_query(db, newSQL, records);   
  }
    
  //Checks for injections with letters / words
  else if (regex_search (sql, match, wordInjection)) {
      std::cout << "SQL Injection Detected: Word match, code removed." << std::endl;
      newSQL = regex_replace(tempString, wordInjection, ";");

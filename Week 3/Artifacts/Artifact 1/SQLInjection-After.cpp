...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //Variables for checking sql injection matches.
  std::string tempString = sql.c_str();       
  std::smatch match;                
  std::string newSQL;                                                                                  
  std::regex numInjectDetection(" or [1-9]+=[1-9]+;");              //Matches if there are any 1=1 type inputs
  std::regex wordInjectDetection(" or '[a-zA-Z]+'='[a-zA-Z]+';");   //Matches if there are any a=a type inputs and/or strings

  //Checks for injections with numbers, if user input matches, message displayed and data replaced with ;
  if (regex_search (tempString, match, numInjectDetection)) { 
      std::cout << "SQL Injection Detected: Number match, code removed." << std::endl;
      newSQL = regex_replace(tempString, numInjectDetection, ";");

      return run_query(db, newSQL, records);   
  }
    
  //Checks for injections with letters / words, if user input matches, message displayed and data replaced with ;
  else if (regex_search (sql, match, wordInjectDetection)) {
      std::cout << "SQL Injection Detected: Word match, code removed." << std::endl;
      newSQL = regex_replace(tempString, wordInjectDetection, ";");

      return run_query(db, newSQL, records);      
  }

  //Displays error message if there is error with getting data from database.
  char* error_message;
